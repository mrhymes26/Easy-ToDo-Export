...
public class AuthService
{
    ...
    private async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            // Windows DPAPI verschlüsselt die Datei mit dem Windows-Benutzerkonto
        ...
        {
            // Fallback: Unverschlüsselt, aber mit Warnung
            Debug.WriteLine("WARNING: Secure token cache not available, using unprotected fallback");
            
            try
            {
                var fallbackProperties = new StorageCreationPropertiesBuilder(CacheFileName, CacheDir)
                    .WithUnprotectedFile()
                    .Build();
                    
                var cacheHelper = await MsalCacheHelper.CreateAsync(fallbackProperties);
                cacheHelper.RegisterCache(_msalClient.UserTokenCache);
            }
            catch
            {
                // App funktioniert auch ohne persistentes Caching
                Debug.WriteLine("Token cache disabled - login required each session");
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<string?> GetAccessTokenAsync()
    {
        ...
        if (_msalClient == null || _authResult == null)
            return null;
        ...
        if (_authResult.ExpiresOn <= DateTimeOffset.UtcNow.AddMinutes(5))
        {
            ...
            {
                // Token widerrufen - neue Anmeldung erforderlich
                await SignInAsync();
            }
        }
